...
by PyTorch's autograd. Three companion scripts share one dataset
(\texttt{common.py}): \texttt{mle\_theoretical.py} checks the closed form
against a brute-force minimization; \texttt{mle\_practical\_pure.py} runs
GD and SGD with hand-derived gradients and \emph{animates} the estimator
descending the NLL landscape; \texttt{mle\_practical\_pytorch.py} verifies,
by driving the two SGD variants through the identical sample schedule,
that autograd reproduces the hand-derived gradient to floating-point
precision.
\end{abstract}
...
within a few $10^{-3}$; and the two SGD trajectories coincide exactly
(measured maximum gap $0.0$ over all $15{,}001$ iterates).
\texttt{mle\_theoretical.py} draws the concave log-likelihood and convex
NLL landscapes with the optimum marked; \texttt{mle\_practical\_pure.py}
plays the animation --- GD's bead gliding down the NLL while SGD's bead
jitters into its noise ball, with the same progress unrolled as an
estimate-versus-time curve alongside; \texttt{mle\_practical\_pytorch.py}
overlays the hand and autograd trajectories (indistinguishable) next to
their per-step gap on a log scale.
