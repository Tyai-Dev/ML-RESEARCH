...
print(f"  95% CI  p̂ ± 1.96 SE      : [{ci_lo:.4f}, {ci_hi:.4f}]"
...
print(f"  mean of p̂  : {mc_mean:.6f}   vs p        = {P_TRUE}"
      f"   (unbiasedness)")
print(f"  var  of p̂  : {mc_var:.3e} vs p(1-p)/n = {var_theory:.3e}"
      f"   (variance = CRLB)")
...
w = 0.35
ax1.bar(np.array([0, 1]) - w / 2, [1 - P_TRUE, P_TRUE], width=w,
        color="#898781", alpha=.8, label=f"true: Bernoulli({P_TRUE})")
ax1.bar(np.array([0, 1]) + w / 2, [1 - p_hat, p_hat], width=w,
        color="#eb6834", alpha=.9, label=f"fitted: Bernoulli({p_hat:.4f})")
ax1.set(xticks=[0, 1], xlabel="x", ylabel="P(X = x)",
        title="the model vs the fit")
...
ax2.plot(p_hat, loglik(p_hat), "o", color="#eb6834",
         label=f"argmax: p̂ = m/n = {p_hat:.4f}")
ax2.set(xlabel="p", ylabel=r"$\ell(p) = m\log p + (n-m)\log(1-p)$",
...
ax3.hist(p_hats, bins=40, density=True, color="#2a78d6", alpha=.55,
         label=f"{R} replicated p̂'s")
zg = np.linspace(p_hats.min(), p_hats.max(), 300)
clt = np.exp(-(zg - P_TRUE) ** 2 / (2 * var_theory)) \
...
ax3.set(xlabel=r"$\hat{p}$", ylabel="density",
        title="the estimator's own distribution")
