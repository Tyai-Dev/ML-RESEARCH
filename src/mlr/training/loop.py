...
a registered model and dataset plus their parameters:

    name: baseline
    topic: linear-discriminator
    model: linear-discriminator
    model_params: {lr: 0.5, epochs: 300}
...
def run_experiment(config: dict, tracker: Tracker) -> int:
    ...
    try:
        ...
        model.fit(X_train, y_train, on_epoch=on_epoch)
