...
The library implements each family twice. The \emph{pure} estimators
(\texttt{bernoulli-mle}, \texttt{multinoulli-mle}, \texttt{gaussian-mle})
apply the formulas above. The \emph{torch} estimators
(\texttt{*-mle-torch}) minimize the NLL~\eqref{eq:nll} by gradient descent
from an uninformed initialization. When the log-likelihood is concave in the
chosen parameterization — true for all three families here — gradient
descent must find the same optimum, so agreement between the two columns of
...
\begin{table}[h]
  ...
  (\texttt{research/mle/experiments/}). Pure rows are closed-form; torch
  rows are Adam on the NLL. Held-out NLL and parameter estimates agree to
  the reported precision, and both recover the generating parameters.}
  \label{tab:estimates}
\end{table}
