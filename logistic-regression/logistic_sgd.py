...
from common import (SEED, evaluate, gradient, make_data, nll, predict,
                    sigmoid)

SGD_LR = 0.05
EPOCHS = 3
PRINT_EVERY = 4_000                          # steps between log lines
...
def train(X, y, X_test, y_test, verbose=True, record=False):
    ...
    step = 0
    if verbose:
        print(f"SGD: lr={SGD_LR}, {EPOCHS} epochs x {n:,} samples = "
              f"{EPOCHS * n:,} steps")
        print(f"{'epoch':>6} {'step':>12} {'train NLL':>10} "
              f"{'test acc':>9}")
    for epoch in range(1, EPOCHS + 1):
        for i in rng.permutation(n):
            ...
            if verbose and step % PRINT_EVERY == 0:
                ...
                print(f"{epoch:>4}/{EPOCHS} {step:>7,}/{EPOCHS * n:,} "
...
if __name__ == "__main__":
    ...
    assert np.linalg.norm(gradient(w, X_train, y_train)) < 0.02, \
