...
fails to exist. All demonstrated in the companion scripts, one solver
per file over a shared \texttt{common.py}: \texttt{logistic\_sgd.py}
(the per-sample loop, logged, with the autograd identity check),
\texttt{logistic\_gd.py} (full gradient; geometric decay of
$\|\nabla\|$ in the log; the separable-data divergence),
\texttt{logistic\_newton.py} (Newton/IRLS; watch $\|\nabla\|$ square
away in eight rows), and \texttt{logistic\_animation.py} (the SGD
film). Each ends with a held-out classification report.
\end{abstract}
...
$p(x)$. The constant choice $p(x)\equiv p$ is the \texttt{Theory/distributions/bernoulli/}
